...
def main():
    ...
    zen_nodes = {hk:hv['ip_address6']
                 for ik, iv in ansible_inventory.items()
                 for sk, sv in iv.items()
                 for hk, hv in sv.items()
                 if isinstance(hv, dict) and 'ip_address6' in hv.keys()
                 and ik in zen_servers and sk == 'hosts'}
    # print(zen_nodes)
    # sys.exit()
    ...
    while rec_len < len(recs):
        rec_len = len(recs)
        recs += cf.zones.dns_records.get(zone_id, params={'type:': 'AAAA', 'page': rec_page, 'per_page': 100})
        rec_page += 1
        # print(len(recs))

    a_recs = {rec['name']: {'id': rec['id'], 'content': rec['content'], 'proxied': rec['proxied']}
              for rec in recs if rec['type'] == 'AAAA'}
    for ar in a_recs:
    ...
    for name, ip in zen_nodes.items():
        ...
        dns_record = {'name':name.split('.')[0], 'type':'AAAA', 'content':ip, 'proxied':False}
        if not a_rec:
            print('Create AAAA record for {}: {}'.format(name, ip))
        ...
        if a_rec and (a_rec['content'] != ip or a_rec['proxied']):
            print('Update AAAA record for {}: {}'.format(name, ip))
            # print(json.dumps(dns_record, indent=2))

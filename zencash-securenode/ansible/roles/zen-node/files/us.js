...
getSetupInfo(url, (err, result) => {
    ...
    if (process.argv.length == 7) {
        ...
        try {
            ...
            let addr = validator(cmdlineArgs[0]);
            let email = cmdlineArgs[1];
            let fqdn = cmdlineArgs[2];
            let ipv = ipvalidator(cmdlineArgs[3]);
            let region = regvalidator(cmdlineArgs[4]);

            localStorage.setItem('stakeaddr', addr);
            localStorage.setItem('email', email);
            localStorage.setItem('fqdn', fqdn);
            localStorage.setItem('ipv', ipv);
            setRegAndServer(region);
            getRPC();
        }
        catch {
            console.log("Unable to validate values specified on command line");
            process.exit(1);
        }
    }
    else {
        ...
        promptly
            ...
            .then((value) => {
                ...
                return promptly.prompt('IP address version used for connection - 4 or 6' + msg4, {
                    'default': ipv,
            ...
            .then((value) => {
                ...
                return promptly.choose('Region code - ' + regPrompt + msg5, regions, {
                    'default': region,
